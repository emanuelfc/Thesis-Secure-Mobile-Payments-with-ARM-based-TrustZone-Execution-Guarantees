...
static long nano_to_milli(long nano)
{
...
static long get_elapsed_time_millis(struct timespec begin, struct timespec end)
{
...
static void evaluate_operation(uint8_t op_code, uint32_t card_id, uint32_t value, char *send_buf, size_t send_buf_size, char *recv_buf, size_t recv_buf_size)
{
	...
	for(int i = 0; i < N_RUNS && res == TEEC_SUCCESS; i++)
	{
		...
		execute_operation(op_code, i, value, send_buf, send_buf_size, recv_buf, recv_buf_size);
